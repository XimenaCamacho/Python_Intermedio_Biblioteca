...
class Accionar_bd:
        ...
        try:
            ...
            cursorObj.execute(
                "CREATE TABLE libros(id integer PRIMARY KEY, titulo text, \
                     descripcion text, autor text, edicion text, isbn text, \
    ...
    def alta(
        ...
        if not self.valido.valida_ingresos(
            ...
            if self.valido.valida_isbn(isbn):
                confirma = messagebox.askquestion("Confirma el ALTA", "Está seguro")
                if confirma == "yes":
                    try:
                        datos = (
                            titulo.get(),
                        ...
                        con = self.conexion()
                        sql = "INSERT INTO libros(titulo, descripcion, autor, edicion, isbn, \
    ...
    def baja(self, tit, mitreeview):
        confirma = messagebox.askquestion("Confirmación de BAJA", "Está seguro")
        if confirma == "yes":
            try:
                datos = (tit.get(),)
                con = self.conexion()
            ...
            if self.valido.valida_isbn(isbn):
                ...
                if confirma == "yes":
                    try:
                        ...
                        con = self.conexion()
                        sql = "UPDATE libros SET descripcion = ?, autor = ?, edicion = ?, \
    ...
    def consultar(
        ...
        confirma = messagebox.askquestion("Confirma la CONSULTA", "Está seguro")
        if confirma == "yes":
            try:
                datos = (
                    titulo.get(),
                ...
                sql = "SELECT * FROM libros WHERE titulo=? OR descripcion=? OR autor=? \
                        OR edicion=? OR isbn=? OR clasificacion=? OR anio=?"
                cur = con.cursor()
